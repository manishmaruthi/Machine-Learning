...
if 1
	pred = zeros(num_movies,num_users);
	error = zeros(num_movies,num_users);
	error_factor = zeros(num_movies,num_users);
	...
	J = (0.5 * sum(sum(error_factor.^2))) + (0.5*lambda*(sum(sum(Theta.^2)))) + (0.5*lambda*(sum(sum(X.^2)))) ;
